...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel is None and after.channel is not None:
        ...
        print(f"🟢 入室: {display_name} → {after.channel.name} ({time_str})")
        log_to_sheet(date, display_name, member.id, after.channel.name, join_time=time_str, leave_time="")
    
    elif before.channel is not None and after.channel is None:
        key = f"{member.id}_{before.channel.id}"
        print(f"🔴 退出: {display_name} ← {before.channel.name} ({time_str})")
        update_leave_time(member.id, before.channel.name, time_str)
        if key in user_join_times:
            del user_join_times[key]
    
    elif before.channel is not None and after.channel is not None and before.channel != after.channel:
        key_before = f"{member.id}_{before.channel.id}"
        print(f"🔄 移動: {display_name} {before.channel.name} → {after.channel.name}")
        update_leave_time(member.id, before.channel.name, time_str)
